...
import { Plus, Search, Filter, MoreHorizontal, Calendar, User, Tag, CheckCircle, Clock, XCircle, AlertCircle, Trash2, Edit2 } from 'lucide-react';
import { useAuth } from './AuthGuard';
import { AccountsPayable, Category } from '../types';
import { formatCurrency, cn } from '../lib/utils';
import { motion, AnimatePresence } from 'motion/react';
import { getAccountsPayable, createAccountsPayable, updateAccountsPayable, deleteAccountsPayable } from '../services/payableService';
...
export function Payable() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateAccountsPayable(editingId, payload);
      } else {
        await createAccountsPayable({ ...payload, created_at: new Date().toISOString() });
      }

      setShowForm(false);
      setEditingId(null);
      setFormData({ due_date: new Date().toISOString().split('T')[0], amount: '', payee: '', category_id: '', notes: '' });
      fetchData();
    } catch (error) {
  ...
  const handleEdit = (bill: AccountsPayable) => {
  ...
  const markAsPaid = async (bill: AccountsPayable) => {
    try {
      await updateAccountsPayable(bill.id, { status: 'paid' });
      // Also create a transaction
      await createTransaction({
        company_id: bill.company_id,
        type: 'expense',
        amount: bill.amount,
        description: `Pagamento: ${bill.payee}`,
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-serif font-bold text-[#1a1a1a]">Contas a Pagar</h2>
          <p className="text-brand/60">Controle suas obrigações e vencimentos.</p>
        </div>
        <button 
          onClick={() => {
            setEditingId(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-brand/5 shadow-sm">
          <p className="text-xs font-bold text-brand/40 uppercase mb-1">Total Pendente</p>
          <h3 className="text-2xl font-bold text-[#1a1a1a]">
            {formatCurrency(bills.filter(b => b.status === 'pending').reduce((acc, b) => acc + b.amount, 0))}
          </h3>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-brand/5 shadow-sm">
          <p className="text-xs font-bold text-red-400 uppercase mb-1">Vencido</p>
          <h3 className="text-2xl font-bold text-red-600">
            {formatCurrency(bills.filter(b => b.status === 'overdue').reduce((acc, b) => acc + b.amount, 0))}
          </h3>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-brand/5 shadow-sm">
          <p className="text-xs font-bold text-green-400 uppercase mb-1">Pago este mês</p>
          <h3 className="text-2xl font-bold text-green-600">
            {formatCurrency(bills.filter(b => b.status === 'paid').reduce((acc, b) => acc + b.amount, 0))}
          </h3>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#f5f5f0]/50 border-bottom border-brand/5">
              ...
              <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-brand/60 font-bold">Favorecido</th>
            ...
            {bills.map((bill) => (
              <tr key={bill.id} className="hover:bg-[#f5f5f0]/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className="text-sm font-semibold text-[#1a1a1a]">{bill.payee}</span>
                </td>
                <td className="px-6 py-4">
                  <span className="text-xs font-medium px-2 py-1 bg-brand/5 text-brand rounded-full">
                    {categories.find(c => c.id === bill.category_id)?.name || 'Despesa'}
                  </span>
                </td>
                <td className="px-6 py-4">
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden">
              <div className="p-8">
                <h3 className="text-2xl font-serif font-bold text-[#1a1a1a] mb-6">{editingId ? 'Editar Conta' : 'Nova Conta a Pagar'}</h3>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-bold text-brand/60 ml-2">Favorecido</label>
                    <input required type="text" value={formData.payee} onChange={(e) => setFormData({...formData, payee: e.target.value})} className="w-full px-4 py-3 bg-[#f5f5f0] border-none rounded-2xl text-sm" placeholder="Ex: Companhia de Energia" />
                  </div>
